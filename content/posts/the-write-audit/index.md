...
We ran exactly that study against our own engine and published the numbers, because we would rather be the ones who found it. Across seven fault classes at ten runs each, screen-level checking silently accepted 50 of 90 wrong outcomes and also aborted 10 runs that had actually succeeded. Moving the consequential step to an effect check against the application's own state took silent acceptance to zero and removed the spurious aborts as well. Your fault mix will differ. The shape of the result usually does not.
